...
foreach my $sub (sort keys %sub) {
	...
	# merge unsorted ublast output, then sort it
	my $blastoutfile = join " ", @blastoutfile;
	`cat $blastoutfile > $mergeblastoutdir/$sub.blast.unsort.txt
	sort -o $mergeblastoutdir/$sub.blast.txt $mergeblastoutdir/$sub.blast.unsort.txt`;
	unlink "$mergeblastoutdir/$sub.blast.unsort.txt";
}
...
`usearch -fastx_mask $in -fastqout $maskfq -qmask $nucleo_masker -threads $threads`;
...
sub mkquerydb { 
	...
	my $in = "$query";
	my $udb = "$query.udb";
	`usearch -makeudb_ublast $in -output $udb -wordlength $wordsize -dbmask $qmasker -dbstep $dbstep`;
...
sub local_usearch {
	...
	my $blastoutunsort = "$blastunsortdir/$fq.blast.unsort.txt";
	`usearch -ublast $input -qmask user -db $querydb -evalue $E -maxaccepts $maxaccepts -maxrejects $maxrejects -userfields target+query+qlo+qhi -userout $blastoutunsort -threads $threads`;
	}
...
print "Start writing parsed reads of $sub\n";
	...
	my ($geneid, $hitid) = $line =~ /(\S+)\s+(\S+)\s+\S+\s+\S+/;
